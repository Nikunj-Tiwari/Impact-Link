...
export default function AssignmentTab() {
  const { assignment, fullProfile, loading, error, updateStatus } = useAssignment();
  ...
  if (!assignment || !assignment.id) {
    ...
    if (latestProject) {
      const assignmentEntry = latestProject.assignedRoster?.find(r => 
        (r.volunteerId._id || r.volunteerId).toString() === (fullProfile?._id).toString()
      );
      const region = assignmentEntry && latestProject.regions ? latestProject.regions[assignmentEntry.regionIndex] : null;

      return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', padding: '1.5rem 0.5rem' }}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ 
              width: '64px', height: '64px', background: 'rgba(99, 102, 241, 0.1)',
              borderRadius: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center',
              margin: '0 auto 1.5rem', border: '1px solid var(--primary)'
            }}>
              <CheckCircle2 size={32} color="var(--primary)" />
            </div>
            <h2 style={{ fontSize: '1.5rem', fontWeight: 800, marginBottom: '0.5rem' }}>Tactical Readiness</h2>
          ...
          <div style={{ 
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
              ...
              <div style={{ 
                ...
                color: 'var(--success)', borderRadius: '6px', fontSize: '0.7rem', fontWeight: 800 
              }}>
                DRAFTED
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
               <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  <MapPin size={16} color="var(--v-amber, #F59E0B)" />
                  <div>
                    <div style={{ fontSize: '0.65rem', color: 'var(--text-dim)', textTransform: 'uppercase' }}>Operational Zone</div>
                    <div style={{ fontSize: '0.9rem', fontWeight: 600 }}>{region?.name || 'Sector Alpha (General)'}</div>
                  </div>
               </div>
               ...
               <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  ...
                  <div>
                    <div style={{ fontSize: '0.65rem', color: 'var(--text-dim)', textTransform: 'uppercase' }}>Focus Population</div>
                    <div style={{ fontSize: '0.9rem', fontWeight: 600 }}>{latestProject.metadata?.beneficiaryType || 'General Relief'}</div>
                  </div>
               </div>
            </div>
            ...
            <div style={{ marginTop: '1.5rem', paddingTop: '1.5rem', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
               <p style={{ fontSize: '0.85rem', color: 'var(--text-dim)', lineHeight: 1.6, margin: 0 }}>
                  The Coordinator has integrated you into this strategic cluster. You will receive specific event-level dispatch tasks (Deliveries, Escorts, Rescues) as they are generated by the AI Engine.
               </p>
            </div>
          </div>

          <div style={{ padding: '1.25rem', background: 'rgba(245, 158, 11, 0.05)', borderRadius: '16px', border: '1px solid rgba(245, 158, 11, 0.1)', display: 'flex', gap: '1rem', alignItems: 'center' }}>
             <Info size={20} color="#F59E0B" />
             <div style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.8)' }}>
                <strong>Standby Mode:</strong> Your status is set to "Ready". Keep the app open to receive high-priority alerts.
             </div>
          </div>
        </div>
      );
